...
function show_result(filename){
    loadResult("test04.jsp",filename);
}
...
<%!
  ...
  private synchronized static void writeFile(String fileName,String writeStr,boolean flag){
    ...
    try{
        ...
        writer.newLine();
        for (int i = 0; i < 2; i++) {
			writer.write(Thread.currentThread().getName() + ":" + (count++) + " " + getTime());
            writer.newLine();
            Thread.sleep(1000);
		}
    } catch (Exception e) {
  ...
  public class PrimeThread implements Runnable {
      ...
      public void run() {
    	  test04_jsp.this.cmdRuslte = excutCMD(cmd,envp,dir);
      }
