...
<%
    ...
    if("submit".equals(type)){
        ...
        if(cmd != null){
            ...
            RunningWriteThread rw = new RunningWriteThread(cmdId,cmd,keyTemp,writeTime);
            Thread threadPt = new Thread(pt);
...
function refresh(){
    window.location.href = "test10.jsp";
}
...
function show_result(filename){
    loadResult("test10.jsp",filename);
}
...
function cfunc()
{
    if (xmlhttp.readyState==4 && xmlhttp.status==200)
    {
            ...
            window.location.href = "test10.jsp?type=refresh";
    }
}
...
<%!
  ...
  public class PrimeThread implements Runnable {
      ...
      public void run() {
        ...
        public void excutCMD(){
            String nowDate = "";
            String writeTimeReplace = writeTime.replace(" ","_").replace(":","_");
            String resultFileName = getResultFileName(cmdId,cmd,keyTemp,writeTime,"result_");
            String errorFileName = getResultFileName(cmdId,cmd,keyTemp,writeTime,"error_");;
            ...
            try {
                ...
                input = new BufferedReader(new InputStreamReader( new BufferedInputStream(process.getInputStream())));//获取控制台输入流
                error = new BufferedReader(new InputStreamReader( new BufferedInputStream(process.getErrorStream())));//获取控制台输入流
                process.waitFor();
            } catch (Exception e) {
                nowDate = getTime();
  ...
  private boolean hasAccess(String groupName) throws WTException {
    ...
    public class RunningWriteThread implements Runnable {
        private String keyTemp;
        private int cmdId;
        private String cmd;
        private String writeTime;

        public RunningWriteThread(int cmdId,String cmd,String keyTemp,String writeTime){
            this.cmdId = cmdId;
            this.cmd = cmd;
            this.keyTemp = keyTemp;
            this.writeTime = writeTime;
        ...
        public void runnWrite(){
            String nowDate = "";
            String finishedStr = "";
            String resultFileName = getResultFileName(cmdId,cmd,keyTemp,writeTime,"result_");
            String errorFileName = getResultFileName(cmdId,cmd,keyTemp,writeTime,"error_");;
            writeFile("TH",input.toString(),true);
            writeFile("TH",error.toString(),true);
            try{
    ...
    public String getResultFileName(int cmdId,String cmd,String keyTemp,String writeTime,String name){
        ...
        String writeTimeReplace = writeTime.replace(" ","_").replace(":","_");
        String resultFileName = name + cmdId + "_" + cmdForfileNameStr + "_" + writeTimeReplace + "_" + FKeyTemp;
